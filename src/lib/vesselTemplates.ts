...
export const VESSEL_TEMPLATES: VesselTemplate[] = [
  {
    ...
    note:
      ...
      'считаются только начальная GM, крен и дифферент. Гидростатика — одна точка (Δ=7456,7 т), за её пределами используется это же табличное значение (без экстраполяции). ' +
